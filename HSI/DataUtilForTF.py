...
def prepareMatList(list, positions,neighbors):
    Data = []
    CId = []
    Positions = []
#    DataTe = []
#    CIdTe = []
    ...
    for sub_list in list:
        ...
        for sub_list_data in sub_list:
            ...
            for to_be_assemblied_data in sub_list_data:
                cols=int(len(to_be_assemblied_data)/neighbors)
                neibordata=np.reshape(to_be_assemblied_data,[neighbors,cols])
                # neibordata=list(neighbors)
                Data.append(neibordata)
                #将类别标签
                Carray=[0 for x in range(13)]
                Carray[classCount-1]=1
                CId.append(Carray)
    ...
    for sub_positions in positions:
        ...
        for sub_sub_positions in sub_positions:
            ...
            #    print str(len(actual_Positions))
                Positions.append(actual_Position[:2])
...
def writeToMAT(trainList, testList,trainPositions, testPositions, datasetName, train_ratio, neighbors):
    DataTr, CIdTr, PositionsTr = prepareMatList(trainList, trainPositions,neighbors)
    DataTe, CIdTe, PositionsTe = prepareMatList(testList, testPositions,neighbors)
    ...
    result_dict = {'TrData': DataTr, 'TrLabel': CIdTr, 'TrPos': PositionsTr, 'TeData': DataTe,'TeLabel': CIdTe, 'TePos': PositionsTe}
    sio.savemat(realPath, result_dict)  # 将数据保存为mat
