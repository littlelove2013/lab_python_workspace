...
x_image=tf.placeholder(tf.float32, [None, h,w,c])
"""
# 第一层
...
"""
W_conv1 = weight_variable([7, 7, c, 32])  # 卷积是在每个5*5的patch中算出32个特征，分别是patch大小，输入通道数目，输出通道数目
...
# 第三层 是个全连接层,输入维数25 * 25 * 64, 输出维数为1024
W_fc1 = weight_variable([56 * 56 * 64, 4096])
...
for i in range(9):
    data=dogbreed.getdata(i)
    imgs = data['images']
    labels = data['labels']
    if (i+1)%4==0:
        train_accuracy = accuracy.eval(feed_dict={
            x_image: imgs, y_: labels, keep_prob: 1.0})
        print("step %d, training accuracy %g" % (i, train_accuracy))
    else:
        print('train batch %d'%(i+1))
        train_step.run(feed_dict={x_image:imgs, y_:labels, keep_prob: 0.5})
data=dogbreed.getdata(10)
imgs=data['images']
labels=data['labels']
